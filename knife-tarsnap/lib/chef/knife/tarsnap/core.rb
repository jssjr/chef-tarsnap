      ...
      module Core
        ...
        def tarsnap_username
          if Chef::Config[:knife][:tarsnap_username]
            ...
            fail StandardError, 'Please provide the tarsnap account username'
          end
        end
        ...
        def tarsnap_nodes
          @_tarsnap_nodes || @_tarsnap_nodes =
            Chef::DataBag.load(tarsnap_data_bag).keep_if { |k, _v| k !~ /^__/ }.map { |k, _v| k.gsub('_', '.') }
        end
        ...
        def which(binary)
          ...
          unless which_shell.status.exitstatus == 0
            fail StandardError, "Unable to locate the tarsnap tool (#{binary}). Is it installed and in your path?"
          end
